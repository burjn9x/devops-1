# Installing & Configuring Camunda-BPM and Alfresco-ECM in AWS Ubuntu 16.04 LTS.
=======================
...
| x | 01 | Nginx        		| 1.10.3             | nginx -v             |                     |
| x | 01 | Oracle Java  		| 1.8.0_151          | java -version        | /etc/java-8-oracle/ |
| x | 01 | MAVEN        		| 3.3.9              | mvn -v               | /usr/share/maven    |
| x | 01 | ANT          		| 1.9.6              | ant -v               | /usr/share/ant      |
| x | 01 | MariaDB      		| 9.4.11             | mysql --version      | DigitalBusiness2018 |
| x | 01 | Tomcat       		| 8.0.38             |         			 	|                     |
| x | 01 | ImageMagick  		| 6.8.9-9            | 						|                     |
| x | 01 | Ghostscript  		| 9.18        		 | 			            |                     |
| x | 01 | Alf PDF Renderer 	| 1.0      			 |             			|                     |
| x | 01 | LibreOffice  		| 5.2.1.2            | libreoffice --version|                     |
| x | 01 | SSL          		|                    |                      |                     |
...
```
 sudo git clone https://DigitalBusiness@bitbucket.org/DevOpsDEC/aws-ubuntu-camunda-alfresco.git &&
 cd ~/aws-ubuntu-camunda-alfresco &&
 sudo chmod u+x camunda-alfresco-install.sh 
...
`/home/devops/alfresco-service.sh` (16.04)  
...
`sudo /home/devops/alfresco-service.sh start` (16.04)
...
The script will shut down Alfresco Tomcat instance. To start it you must call `sudo /home/devops/alfresco-service.sh start` (16.04) or `sudo start alfresco` (14.04).  
...
The `maintenance.html` page is found in its default location /home/devops/www and can be customized to your needs.  
...
========
A special directory is created, `/home/devops/addons`. This directory can be used to manage any addons and the core war files.  
...
============================
In the directory `/home/devops/scripts` there are some useful scripts installed. Or if you did not run the install script, grab them from github. Here is what they do:  
...
`*/10 * * * * /home/devops/scripts/libreoffice.sh start 2>&1 >> /home/devops/logs/office.log`  
`0 2 * * * /home/devops/scripts/libreoffice.sh restart 2>&1 > /home/devops/logs/office.log`  
